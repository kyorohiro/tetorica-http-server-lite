...
lipo -create \
  zig-out/aarch64-macos/bin/tetorica_http_server_lite \
  zig-out/x86_64-macos/bin/tetorica_http_server_lite \
  -output dist-cli-universal/tetorica_http_server_lite
...
codesign \
  ...
  --options runtime \
  --sign "$APPLE_SIGNING_IDENTITY" \
  dist-cli-universal/tetorica_http_server_lite
...
tar -czf "tetorica_http_server_lite-${VERSION}-universal-macos.tar.gz" -C dist-cli-universal tetorica_http_server_lite
...
shasum -a 256 "tetorica_http_server_lite-${VERSION}-universal-macos.tar.gz"
